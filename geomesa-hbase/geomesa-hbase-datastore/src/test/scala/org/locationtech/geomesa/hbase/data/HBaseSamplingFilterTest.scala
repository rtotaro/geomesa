    ...
    "working with sampling" in {
      ...
      try {
...

        {
          ...
          val stat:CountStat = StatsScan.decodeStat(sft)(ds.getFeatureReader(query, Transaction.AUTO_COMMIT).next.getAttribute(0).asInstanceOf[String]).asInstanceOf[CountStat]
